...
---------------------------------------------------------
-- [NEW ADDITIONS: AUTOMATION & CONSUMABLES]
---------------------------------------------------------
mainTab:AddLabel("--- Wheel & Consumables ---")
    ...
    if autoSpinActive then
        task.spawn(function()
            while autoSpinActive do
                ...
                if rEvents then
                    local wheelRemote = rEvents:FindFirstChild("wheelRemote") or rEvents:FindFirstChild("spinWheelRemote")
                    if wheelRemote then
...
-- Auto Eat Egg Every 30 Minutes
local autoEatEggActive = false
mainTab:AddSwitch("Auto Eat Egg (Every 30 Mins)", function(state)
    autoEatEggActive = state
    if autoEatEggActive then
        task.spawn(function()
            while autoEatEggActive do
                local rEvents = game:GetService("ReplicatedStorage"):FindFirstChild("rEvents")
                if rEvents and rEvents:FindFirstChild("eatItemRemote") then
                    rEvents.eatItemRemote:InvokeServer("Egg")
                end
                task.wait(1800) -- 30 minutes interval
            end
        end)
    end
end)

-- Helper function for eating consumables continuously
local function createConsumableSwitch(switchName, itemName)
    local active = false
    mainTab:AddSwitch(switchName, function(state)
        active = state
        if active then
            task.spawn(function()
                while active do
                    local rEvents = game:GetService("ReplicatedStorage"):FindFirstChild("rEvents")
                    if rEvents and rEvents:FindFirstChild("eatItemRemote") then
                        rEvents.eatItemRemote:InvokeServer(itemName)
                    end
                    task.wait(0.5)
                end
            end)
...
createConsumableSwitch("Auto Eat Protein Bar", "Protein Bar")
createConsumableSwitch("Auto Eat Energy Shake", "Energy Shake")
createConsumableSwitch("Auto Eat Toughbar", "Toughbar")
createConsumableSwitch("Auto Eat Energy Bar", "Energy Bar")
createConsumableSwitch("Auto Eat Ultra Shake", "Ultra Shake")
createConsumableSwitch("Auto Eat Protein Shake", "Protein Shake")

mainTab:AddLabel("--- Farm Mechanics ---")
